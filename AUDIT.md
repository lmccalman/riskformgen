...
### 3.2 ⚠ `CLAUDE.md` still describes the removed graph visualisation
Commit `f252325` ("remove graph visualisation for now") deleted
`graph.py`, `templates/graph.html.j2`, and `assets/panzoom.min.js`. The
dependency on `grandalf` is also gone from `pyproject.toml`. But `CLAUDE.md`
still contains:

- `CLAUDE.md:52` — "`graph.py` computes a Sugiyama layered layout … using
  `grandalf`."
- `CLAUDE.md:80, 84` — key-files table entries for `graph.py` and
  `templates/graph.html.j2`.
- `CLAUDE.md:83` — "Page skeleton with … panzoom init."
- `CLAUDE.md:93-102` — a whole "Graph visualisation" section.
- `CLAUDE.md:140` — reference to `templates/graph.html.j2` in the autoescape
  gotcha.
- `CLAUDE.md:153-155` — `# pyright: reportAttributeAccessIssue=false` override
  for `graph.py` (no such file).
- `CLAUDE.md:159` — `panzoom.min.js` listed as a copied output asset.
- `CLAUDE.md` "Form structure" paragraph — claims "The Graph tab (blue accent)
  and Risk Analysis tab (red accent, right-aligned) are always present." Only
  Risk Analysis (and the Debug tab — undocumented) are present now.
...
2. **Rewrite `README.md`** (§3.1) — the current file actively misleads. In the
   ...
   quickstart.
3. **Prune `CLAUDE.md`** of all graph-visualisation references (§3.2) — one
   pass, high signal, prevents future confusion.
